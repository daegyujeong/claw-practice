...
> Nomad Coders 「Cloudflare Agents」 강의 Section 3의 학습 노트다. 지금은 3.0(셋업) ~ 3.3(WebSocket 이벤트)까지 정리했고, 섹션이 진행되면 이어서 채운다.
...
3.2 호출: @callable 데코레이터 → 프론트에서 agent.stub.메서드() 호출
          + 프론트의 setState 직접 호출(override)과 onStateChange(source)
3.3 소켓: 채팅방으로 리팩터링 — onConnect / onClose / onMessage
...
## 4. 3.2 — @callable: 프론트에서 에이전트 메서드 호출
...
프론트도 `agent.setState({ pingPongCount: 10000 })`처럼 **서버 메서드를 거치지 않고** 상태를 바꿀 수 있다. 강의는 이걸 override라고 불렀다. 되긴 되지만 권장하지 않는다 — 지금은 **아무 클라이언트나 상태를 마음대로 바꿀 수 있는 보안 구멍**이기 때문이다. 이상적으로는 `@callable` 메서드를 만들고 그 안에서 검증하라고 했고, read-only connections는 뒤 강의에서 다룬다.

### onStateChange — 누가 바꿨는지 추적
...
- 프론트가 `agent.setState`로 직접 바꾸면(override) `source`는 **그 WebSocket 연결** — 상태 변경의 출처가 서버 바깥이라는 뜻이다.

> 📘 공식 문서 보강 — 현재 공식 문서 기준 이 콜백의 이름은 **`onStateChanged(state, source)`** 다(강의 시점 SDK와 이름이 다를 수 있으니 자동완성으로 확인). 이 콜백은 브로드캐스트가 끝난 **뒤에** 불린다 — 감시용이지 차단용이 아니다. 클라이언트발 변경을 **막으려면** `validateStateChange(nextState, source)`를 쓴다: 저장 전에 동기적으로 실행되고, 여기서 throw하면 변경이 거부된다. read-only connections가 나오기 전에도 이걸로 override를 방어할 수 있다.

### onStateUpdate가 "필요 없다"고 한 이유

프론트의 `onStateUpdate` 콜백으로 상태를 React state에 복사하지 않아도, JSX에서 `agent.state.pingPongCount`를 **직접 참조**하면 상태가 바뀔 때 화면이 갱신된다. 즉 콜백은 필수가 아니라 **"상태가 바뀌는 순간마다 뭔가 하고 싶을 때"**(로그, 알림, 파생 계산) 쓰는 선택지다.

---

## 5. 3.3 — 채팅방 에이전트와 WebSocket 이벤트
...
| 메서드 | 언제 불리나 | 강의에서 한 일 |
|---|---|---|
| `onConnect(connection)` | 새 클라이언트가 WebSocket으로 연결됐을 때 | `setState`로 `currentlyOnline + 1` |
| `onClose(connection)` | 연결이 끊겼을 때 | `currentlyOnline - 1` |
...
- **에이전트 → 전원**: `setState`(상태 브로드캐스트) 또는 📘 `this.broadcast(...)`.
- **프론트에서 수신**: `useAgent`의 `onMessage: (event) => ...` 옵션 — 내용은 `event.data`에 있다. `console.log(event)`로 구조를 확인할 수 있다.

> 📘 공식 문서 보강 — 정확한 시그니처와 추가 도구들:
> - `onConnect(connection, ctx)` — `ctx.request`로 요청 헤더·쿠키에 접근할 수 있어 **인증 후 `connection.close()`로 연결을 거절**하는 자리다.
...
이 채팅방에는 아직 AI가 없다 — 이 챕터는 에이전트의 원시 기능(primitives)을 익히는 단계이고, AI는 다음 강의에서 붙인다.
...
| 파일 | 역할 |
|---|---|
| `worker/index.ts` | `ChattingRoomAgent`(state + `@callable` increment/decrement) + `routeAgentRequest` 라우팅 |
| `src/App.tsx` | `useAgent` 훅으로 연결 → 카운트 실시간 표시 + stub 호출/override 버튼 |
...
- 3.1에서 임시로 남겨 뒀던 `void agent;` 참조는 3.2에서 `agent.stub` 호출이 생기면서 제거했다.
- 3.3의 채팅방 리팩터링(`ChattingRoomState`, onConnect/onClose/onMessage, 메시지 폼)은 **아직 코드에 반영하지 않았다** — 다음 실습에서 핑퐁 코드를 대체할 예정이다.
- 이 카운터가 학습용 예제로 좋은 이유: Section 1의 KV 카운터가 가진 한계(초당 1회 쓰기, 최종 일관성, 레이스 컨디션)를 **DO의 단일 인스턴스 + 자동 브로드캐스트**가 어떻게 해결하는지 가장 작은 코드로 보여준다. 단, override 버튼이 보여주듯 **쓰기 권한 제어가 아직 없다**는 것이 이 예제의 의도된 한계다.
...
6. **프론트의 `agent.setState` 직접 호출(override)은 가능하지만 보안 구멍이다.** `onStateChange(d)`의 `source`("server" vs WebSocket 연결)로 출처를 구분하고, 📘 `validateStateChange`로 저장 전에 거부할 수 있다.
...
지금은 모든 클라이언트가 같은 권한으로 연결되고, 누구나 override로 상태를 덮어쓸 수 있다. 그래서 뒤 강의에서 **read-only connections**로 쓰기 권한을 제한하는 방법이 나오고, 이 채팅방 위에 **AI를 얹어** 진짜 에이전트로 만든다. 다음 실습은 3.3의 채팅방 리팩터링을 코드에 반영하는 것부터다.
